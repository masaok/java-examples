...
public class RobotSoccerSimulation extends JPanel {
    ...
    static class Ball {
        ...
        boolean inside(Goal goal) {
            return this.x - this.radius > goal.x - goal.w / 2 && this.x + this.radius < goal.x + goal.w / 2
                    && this.y - this.radius > goal.y - goal.h / 2 && this.y + this.radius < goal.y + goal.h / 2;
        }
    }
    private static Ball[] balls = new Ball[] { new Ball(0.0, HEIGHT, PLAYER_RADIUS, PLAYER_SPEED, Color.BLUE),
